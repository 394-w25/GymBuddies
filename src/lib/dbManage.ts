import { database, User, Workout } from "@/lib/firebase" // Your Firebase config file
...
export const addWorkout = async (
  ...
  try {
    ...
    const userWorkouts = userWorkoutsSnapshot.exists()
      ? [...userWorkoutsSnapshot.val(), workout_id]
      : [workout_id]
...
export const readWorkout = async (workoutId: string) => {
  try {
    ...
    console.log(
      `an error ocurred while trying to lookup workout ${workoutId}`,
      err
    )
    return {}
  }
